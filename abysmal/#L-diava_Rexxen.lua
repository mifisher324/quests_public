...
function event_say(e)
  if e.other:IsTaskCompleted(bic_abysmal) or (e.other:IsTaskCompleted(bic_yxtta) and e.other:IsTaskCompleted(bic_kodtaz)) then
...
function event_trade(e)
  ...
  if item_lib.check_turn_in(e.trade, {item1 = 67562}) then --Sealed Confession
    ...
    e.other:UpdateTaskActivity(bic_kodtaz, 4, 1)
    e.other:UpdateTaskActivity(bic_abysmal, 9, 1)
  end
  ...
  if item_lib.check_turn_in(e.trade, {item1 = 67561}) then --Kitren's Tattered Cloak
    ...
    e.other:UpdateTaskActivity(bic_kodtaz, 0, 1)
