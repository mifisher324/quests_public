...
function event_trade(e)
  ...
  if item_lib.check_turn_in(e.trade, {item1 = 67520}) then
    if fer_bic == 2 then
      ...
      e.other:SetBucket("god.bic.ferubi", "3")
    else
      item_lib.return_items(e.self, e.other, e.trade)
    end
  end

  if item_lib.check_turn_in(e.trade, {item1 = 67526}) then
    if fer_bic == 4 then
      e.other:Message(MT.NPCQuestSay, "You have now finished the scouting of the city.  Take this as a reward.")
      e.other:SummonItem(67527)
      e.other:SetBucket("god.bic.ferubi", "5")
    else
      item_lib.return_items(e.self, e.other, e.trade)
    end
  end
  ...
  if item_lib.check_turn_in(e.trade, {item1 = 67401}) then
    if qin_bic == 5 then
      ...
      e.other:SetBucket("god.bic.qinimi", "6")
    else
      item_lib.return_items(e.self, e.other, e.trade)
    end
  end
